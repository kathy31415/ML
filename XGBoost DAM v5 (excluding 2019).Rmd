...
suppressWarnings(requiredpackages <- c("readxl", "stringr", "tidyverse", "ggplot2", "dplyr", "digest", "xgboost", "zoo", "vtreat", "magrittr", "rapport"))
...
```{r}
#Import data
suppressWarnings(data <- read_xlsx("//gopowerfp1/LCC Power/Kathy.Callan/Ex Ante Prediction/Data/All Data - DAM - Imputed - Copy.xlsx", col_names = TRUE)[4417:37512,1:15])
...
#write.csv(outcomes, "gas model exc 2019.csv")
```

```{r facet wrapped plots}
...
write.csv(outcomes, "Sep1to7excl2019.csv")
```
...
write.csv(outcomes, "Sep8to14excl2019.csv")
```
...
write.csv(outcomes, "Sep15to21excl2019.csv")
```
...
write.csv(outcomes, "sep22to28excl2019.csv")
```
...
write.csv(outcomes, "sep29oct05excl2019.csv")
```

```{r 9th-15th Oct FinAn}

#get new data.test
suppressWarnings(oct9to15 <- read_xlsx("//gopowerfp1/LCC Power/Kathy.Callan/Ex Ante Prediction/Data/All Data - DAM - Imputed - Copy.xlsx", col_names = TRUE)[37465:37632,c(1:11,14,15)])
colnames(oct9to15) <- c('Year', 'Date', 'Month', 'Hour', 'PriceDAM', 'VolDAM', 'DOY', 'DOM', 'DOW', 'WOY', 'Quarter', 'ForecastWind', 'Gas')
vars <- c('Year', 'Date', 'Month', 'Hour', 'DOY', 'DOM', 'DOW', 'WOY', 'Quarter', 'ForecastWind', 'Gas')

#glimpse(oct9to15)

newdata <- oct9to15 %>% select(-c(PriceDAM, VolDAM))

data.test <- prepare(train, newdata, varRestriction = newvars)
data.test$pred <- predict(model, as.matrix(data.test))

outcomes <- cbind("Actual" = oct9to15$PriceDAM, "Predicted" = data.test$pred, "Date" = paste0(data.test$DOM," ",data.test$Month," ",data.test$Year), Hour = data.test$Hour)
#write.csv(outcomes, "oct9to15.csv")

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#not good accuracy - try retraining with more recent data!
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
```


```{r}
#Import data again - use difference dates (more/less?)
suppressWarnings(data <- read_xlsx("//gopowerfp1/LCC Power/Kathy.Callan/Ex Ante Prediction/Data/All Data - DAM - Imputed - Copy.xlsx", col_names = TRUE)[((24*7*4*6)+4417):37512,1:15])
...
data.test$pred <- predict(model, as.matrix(data.test))
outcomes <- cbind("Actual" = data$PriceDAM[(nrow(data.train)+1):nrow(data)], "Predicted" = data.test$pred, "Date" = paste0(data.test$DOM," ",data.test$Month," ",data.test$Year," ",data.test$Hour))
write.csv(outcomes, "gas model retrained.csv")

#get new data.test
suppressWarnings(oct9to15retrained <- read_xlsx("//gopowerfp1/LCC Power/Kathy.Callan/Ex Ante Prediction/Data/All Data - DAM - Imputed - Copy.xlsx", col_names = TRUE)[37465:37632,c(1:11,14,15)])
colnames(oct9to15retrained) <- c('Year', 'Date', 'Month', 'Hour', 'PriceDAM', 'VolDAM', 'DOY', 'DOM', 'DOW', 'WOY', 'Quarter', 'ForecastWind', 'Gas')
vars <- c('Year', 'Date', 'Month', 'Hour', 'DOY', 'DOM', 'DOW', 'WOY', 'Quarter', 'ForecastWind', 'Gas')

#glimpse(oct9to15)

newdata <- oct9to15retrained %>% select(-c(PriceDAM, VolDAM))

data.test <- prepare(train, newdata, varRestriction = newvars)
data.test$pred <- predict(model, as.matrix(data.test))

outcomes <- cbind("Actual" = oct9to15retrained$PriceDAM, "Predicted" = data.test$pred, "Date" = paste0(data.test$DOM," ",data.test$Month," ",data.test$Year), Hour = data.test$Hour)
write.csv(outcomes, "oct9to15retrained.csv")
```
