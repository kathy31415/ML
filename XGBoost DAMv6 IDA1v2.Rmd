...
requiredpackages <- c("caret", "corrplot", "data.table", "digest", "dplyr", "ggplot2", "gridExtra", "Hmisc", "magrittr", "plyr", "psych", "rapport", "readxl", "stringr", "tidyverse", "vtreat", "xgboost", "zoo")
...
# read in from 1/1/19 to 15/10/23
...
# yearcat <- ggplot(data, aes(x=Year, y = PriceDAM)) +
#   geom_bar(stat='summary', fun.y = 'median', fill = 'blue') +
#   geom_hline(yintercept = mean(data$PriceDAM), linetype = 'dashed', color = 'orange')
# monthcat <- ggplot(data, aes(x=Month, y = PriceDAM)) +
#   geom_bar(stat='summary', fun.y = 'median', fill = 'blue') +
#   geom_hline(yintercept = mean(data$PriceDAM), linetype = 'dashed', color = 'orange')
# domcat <- ggplot(data, aes(x=DOM, y = PriceDAM)) +
#   geom_bar(stat='summary', fun.y = 'median', fill = 'blue') +
#   geom_hline(yintercept = mean(data$PriceDAM), linetype = 'dashed', color = 'orange')
# hourcat <- ggplot(data, aes(x=Hour, y = PriceDAM)) +
#   geom_bar(stat='summary', fun.y = 'median', fill = 'blue') +
#   geom_hline(yintercept = mean(data$PriceDAM), linetype = 'dashed', color = 'orange')
# dowcat <- ggplot(data, aes(x=DOW, y = PriceDAM)) +
#   geom_bar(stat='summary', fun.y = 'median', fill = 'blue') +
#   geom_hline(yintercept = mean(data$PriceDAM), linetype = 'dashed', color = 'orange')
# woycat <- ggplot(data, aes(x=WOY, y = PriceDAM)) +
#   geom_bar(stat='summary', fun.y = 'median', fill = 'blue') +
#   geom_hline(yintercept = mean(data$PriceDAM), linetype = 'dashed', color = 'orange')
# quartercat <- ggplot(data, aes(x=Quarter, y = PriceDAM)) +
#   geom_bar(stat='summary', fun.y = 'median', fill = 'blue') +
#   geom_hline(yintercept = mean(data$PriceDAM), linetype = 'dashed', color = 'orange')

# suppressMessages(grid.arrange(yearcat, monthcat, domcat, hourcat, dowcat, woycat, quartercat, widths=c(2,4)))
...
#cat('There are', length(numericVars), 'numeric variables, and', length(factorVars), 'factored categorical variables')
```
...
CorHigh <- names(which(apply(cor_sorted, 1, function(x) abs(x)>0.00000000000000002)))
cor.dam <- cor.dam[CorHigh, CorHigh]
corrplot.mixed(cor.dam, tl.col="black", tl.pos = "lt")

# for highly correlated predictor variables, remove the one that correlates less strongly with PriceDAM
...
```{r normalise skewed numeric variables}

# list of num and categ variables
numericVarNames <- c('DOY', 'Gas', 'ForecastWind')
DFnumeric <- data[, names(data) %in% numericVarNames]
DFcategorical <- data[, !names(data) %in% numericVarNames & !names(data) %in% c("PriceDAM")]
...
# ```{r one hot encoding of categorical variables}

# create multiple columns with 1s and 0s
...
# data are slightly right-skewed but gte worse with transformations so leave as it
# ```
...
data.dam.test <- data[30721:37608,]
...
outcomes <- cbind("Actual" = data$PriceDAM[(nrow(data.dam.train)+1):(nrow(data)-24)], "Predicted" = data.dam.test$pred, "Date" = paste0(data.dam.test$DOM," ",data.dam.test$Month," ",data.dam.test$Year," ",data.dam.test$Hour))
...
# feats <- c(importance.dam$Feature[importance.dam$Importance > quantile(importance.dam$Importance, 0.5)], 'PriceDAM')

# select only top 5 important features as pred vars
sorted <- importance.dam[order(-importance.dam$Importance),]
feats <- c(sorted[1:5]$Feature, 'PriceDAM')
...
data.dam.test <- data[37609:37632,]
...
# Plot feature importance matrix
importance.dam <- xgb.importance(feature_names <- as.array(newvars.dam), model = model.dam)
xgb.ggplot.importance(importance_matrix = importance.dam)
...
write.csv(outcomes, "//gopowerfp1/LCC Power/Kathy.Callan/Ex Ante Prediction/XGBoost Models/Output/DAMv6IDA1v2 - DAM Predict 151023 with Top 5 One Hot Coded Variables.csv")
```
