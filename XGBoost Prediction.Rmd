...
#Read in data
suppressWarnings(data <- read_xlsx("//gopowerfp1/LCC Power/Kathy.Callan/Ex Ante Prediction/Data/All Data - DAM - Imputed.xlsx", col_names = TRUE)[,1:22])
data <- select(data, -c('Temporary1':'wdsp Tipp'))
colnames(data) <- c('Year', 'Date', 'Hour', 'PriceDAM', 'VolDAM', 'DOY', 'DOW', 'tempAvg', 'wdspAvg', 'forecastWind')
...
data <- data[, c(2,3,11,14,13,1,7,12,6,4,5,8,9,10)]
...
test <- anti_join(data, train2)
nrow(train2) + nrow(test) == nrow(data) #sanity check
...
```
```{r}
# retrain with importance feats only
train2 <- train[, colnames(train) %in% c("PriceDAM", "rollmean_2", "rollmean_3")]
```
...
writexl::write_xlsx(results, "//gopowerfp1/LCC Power/Kathy.Callan/Ex Ante Prediction/XGBoost Models/PredictionOutput3.xlsx")
```
